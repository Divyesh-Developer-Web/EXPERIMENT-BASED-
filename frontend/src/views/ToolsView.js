...
import { listTools, listMemories, clearMemories, deleteMemory } from "@/lib/api";
...
export default function ToolsView() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [t, m] = await Promise.all([listTools(), listMemories()]);
      setTools(t || []);
      setMemories(m || []);
    } finally {
      setLoading(false);
    ...
    <div className="px-6 md:px-10 lg:px-16 pt-8 max-w-6xl mx-auto w-full">
      <div className="mb-10">
        ...
        <h1 className="font-display text-4xl md:text-5xl font-bold tracking-tight">
          Tool hub.
          <span className="text-white/40"> What Nova can do.</span>
        </h1>
        <p className="font-mono text-sm text-white/50 mt-4 max-w-2xl">
          Each tool is a Python function on the backend that Claude can call via our
          MCP-style protocol. Mocked tools return sample data — swap them for real
          APIs (OpenWeatherMap, Serper, Google Calendar…) by editing{" "}
          <code className="text-white/80">backend/server.py</code>.
        </p>
      </div>

      <div className="grid md:grid-cols-2 gap-4 mb-12" data-testid="tools-grid">
